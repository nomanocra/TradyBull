...
MIN_SLOPE_PCT = 0.01  # MA200 must have risen by at least 0.01%


class DailySL1TrendSlope01Strategy(BaseStrategy):
    """
    Daily SL-1% + Trend Filter Strategy with Minimum Slope 0.1%

    Entry:
    - First candle where price > MA200 AND MA200 rising by at least 0.1% over 20h
    ...
    @property
    def name(self) -> str:
        return "daily-sl1-trend-slope01"

    @property
    def display_config(self) -> StrategyDisplayConfig:
        return StrategyDisplayConfig(
            display_name="Multi-Daily SL-1% Trend200 Slope 0.01% IntraD",
            description="Entry: price > MA200 and MA200 rising >= 0.01% over 20h. Exit: SL -1%, trend break, or 22h.",
            ...
            if open_position is None:
                if self._check_trend_bullish(closes, ma200, i) and not is_too_close_to_close(candle['time']):
                    signals.append(Signal(
                        ...
                        price=candle['open'],
                        label='Buy',
                        metadata={'ma200': ma200[i], 'trend_filter': 'slope_0.01%'}
                    ))
                    open_position = {
