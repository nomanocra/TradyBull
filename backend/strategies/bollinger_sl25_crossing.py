...
class BollingerSL25CrossingStrategy(BaseStrategy):
    """
    Bollinger SL-2.5% + Golden Cross Filter Strategy
    ...
    Golden Cross Filter:
    - Only buy if MA50 > MA200 (Golden Cross is active)
    """

    @property
    def name(self) -> str:
        return "bollinger-sl25-crossing"

    @property
    def display_config(self) -> StrategyDisplayConfig:
        return StrategyDisplayConfig(
            display_name="Bollinger SL-2.5% Crossing IntraD",
            description="Entry: price touches lower BB + MA50 > MA200. Exit: SL -2.5% or end of day.",
            show_bollinger=True,
    ...
    def _check_golden_cross(self, ma50: List[Optional[float]], ma200: List[Optional[float]], index: int) -> bool:
        """
        Check if Golden Cross is active: MA50 > MA200
        """
