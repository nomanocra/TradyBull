...
class Daily1xSL1TrendStrategy(BaseStrategy):
    """
    Daily (1x/day) SL-1% + Trend Filter Strategy

    Entry:
    - First candle where price > MA200 AND MA200 rising
    - ONE trade per day max (no re-entry after exit)
    ...
    @property
    def name(self) -> str:
        return "daily-1x-sl1-trend"

    @property
    def display_config(self) -> StrategyDisplayConfig:
        return StrategyDisplayConfig(
            display_name="Daily SL-1% Trend MA200",
            description="Entry: price > MA200 and MA200 rising (1x/day). Exit: SL -1%, trend break, or 22h.",
    ...
    def calculate_signals(
        ...
        # Track position and whether we already traded today
        if initial_state:
            open_position = initial_state.get('open_position', None)
            traded_today = initial_state.get('traded_today', None)  # Date string of last trade
        else:
            open_position = None
            traded_today = None
        ...
        for i in range(start_index, len(candles)):
            ...
            # Check for buy signal (only if no open position AND haven't traded today)
            ...
            if open_position:
                ...
                if self._check_stop_loss(candle, buy_price):
                    ...
                    open_position = None
                    traded_today = date_string  # No more trades today
                    continue
