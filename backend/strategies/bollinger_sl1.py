...
class BollingerSL1Strategy(BaseStrategy):
    """
    Bollinger SL-1% Strategy
    ...
    @property
    def name(self) -> str:
        return "bollinger-sl1"

    @property
    def display_config(self) -> StrategyDisplayConfig:
        return StrategyDisplayConfig(
            display_name="Bollinger SL-1% IntraD",
            description="Entry: 7h-21h, price touches lower BB. Exit: SL -1% or 22h.",
            show_bollinger=True,
        )
    ...
    def calculate_signals(
        ...
        for i in range(BB_PERIOD, len(candles)):
            ...
            if low_below_band and is_in_trading_hours and not last_signal_triggered and not already_traded_today and not is_too_close_to_close(candle['time']):
